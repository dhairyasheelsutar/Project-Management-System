...
@WebServlet("/review_2_checklist")
public class review_2_checklist extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public review_2_checklist() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int j=0;
		for(int i=16;i<32;i++) {
			values[i-16] = request.getParameter("check1list" + i);
		}
		...
		for(int i=16;i<32;i++) {
			j = i+1;
			query += " when id = " + j + " then " + values[i-16];
		}
		query += " end ) where type = 2";
		String output = "";
		
		
		System.out.println(query);
		if(database.dml(query) != -1) {
			output = functions.set_output_message("Review 2 checklist updated", true);
		}else {
